...
led_flash()
{
	if [ "$led_flash_pid" -gt "0" ]; then kill -9 $led_flash_pid >/dev/null 2>&1; fi
	(while true; do
		i=2
		while [ "$i" -le "$#" ]; do
			eval "arg=\${$i}"
...
kill9_pid_tree()
{
	...
	ps l | grep -oE "[0-9]+[ \t]+$1[ \t]" | grep -oE "^[0-9]+" | while read -r line; do
			if [ "$line" -gt "0" ]; then kill -9 $line >/dev/null 2>&1; fi
		done
	...
	if [ "$pow_pid" -eq "0" ]; then
		...
		if [ "$platform" == "mpp" ]; then
			(evtest /dev/input/event0 | grep -m 1 -e "type 1 (EV_KEY), code 295 (BTN_BASE2), value 1" | while read; do
				...
				killall -SIGCONT mppd >/dev/null 2>&1
			done) &
			pow_pid=$!
		else
...
main()
{
	...
	zerotier_iface_timeout_seconds=10
	...
	echo wifi >/tmp/mode
	echo wifi >/tmp/uavpal_sc2_mode
	echo 0 >/tmp/button_prev_timestamp
	echo 0 >/tmp/button_timestamp
	write_sc2_diag wifi "" ""
...
{
	echo lte >/tmp/mode
	echo lte >/tmp/uavpal_sc2_mode
	rm -f /tmp/zt_interface
	rm -f /tmp/zerotier-one_err
	sc2_last_error=""
	write_sc2_diag lte "" ""
	...
	killall -SIGSTOP mppd >/dev/null 2>&1
	...
	if [ "$platform" == "mpp" ]; then
		...
		killall -SIGSTOP wifid >/dev/null 2>&1
	...
	for p in `seq 1 $wifi_connection_attempts`; do
		...
		if [ "$p" -eq "$wifi_connection_attempts" ]; then
			...
			sc2_last_error="wifi_connection_failed"
			write_sc2_diag lte "" "$sc2_last_error"
	...
	do
		...
		if [ "$p" -eq "$wifi_dhcp_timeout_seconds" ]; then
			...
			sc2_last_error="wifi_dhcp_failed"
			write_sc2_diag lte "" "$sc2_last_error"
			kill9_pid_tree $pow_pid
			pow_pid=0
			switch_to_wifi
	...
	if [ "$platform" == "mpp" ]; then
		...
		wifid_uavpal_pid=`ps |grep '[w]ifid' |grep suffix |awk '{print $1}'`
		if [ -n "$wifid_uavpal_pid" ]; then kill -9 $wifid_uavpal_pid >/dev/null 2>&1; fi
	fi
	...
	if [ -d "/data/lib/zerotier-one/networks.d" ] && [ ! -f "/data/lib/zerotier-one/networks.d/$(conf_read zt_networkid).conf" ]; then
		...
		ln -s /data/lib/ftp/uavpal/conf/local.conf /data/lib/zerotier-one/local.conf
	fi

	if ps |grep '[z]erotier-one' >/dev/null 2>&1; then
		ulogger -s -t uavpal_sc2 "... zerotier daemon already running"
	else
	...
	for p in `seq 1 $zerotier_iface_timeout_seconds`; do
		...
		if [ "$platform" == "mpp2" ] && [ -n "$zt_interface" ]; then
		...
		if [ -n "$zt_interface" ] && route -n | awk -v iface="$zt_interface" '$1=="192.168.42.0" && $8==iface { found=1 } END { exit !found }'; then
			ulogger -s -t uavpal_sc2 "... IP route for drone via zerotier interface $zt_interface already exists"
			echo $zt_interface >/tmp/zt_interface
			write_sc2_diag lte "$zt_interface" ""
			break # break out of loop
		fi
		if [ -n "$zt_interface" ] && route add -net 192.168.42.0 netmask 255.255.255.0 dev $zt_interface >/dev/null 2>&1; then
		...
		if [ "$p" -eq "$zerotier_iface_timeout_seconds" ]; then
			...
			sc2_last_error="zt_route_failed"
			write_sc2_diag lte "$zt_interface" "$sc2_last_error"
			kill9_pid_tree $pow_pid
			pow_pid=0
			switch_to_wifi
			return # back to main()
	...
	for p in `seq 1 $drone_zt_ping_retry`; do
		...
		if [ "$p" -eq "$drone_zt_ping_retry" ]; then
			...
			sc2_last_error="drone_zt_ping_failed"
			write_sc2_diag lte "$zt_interface" "$sc2_last_error"
			kill9_pid_tree $pow_pid
			pow_pid=0
			switch_to_wifi
	...
	if [ "$platform" == "mpp2" ]; then
		...
		killall -SIGSTOP wifid >/dev/null 2>&1
		if [ "$led_flash_pid" -gt "0" ]; then kill -9 $led_flash_pid >/dev/null 2>&1; fi # stop blinking blue
		led_flash_pid=0
	fi
...
switch_to_wifi()
{
	...
	if [ "$platform" == "mpp" ]; then
		...
		sleep 1
		if [ "$led_flash_pid" -gt "0" ]; then kill -9 $led_flash_pid >/dev/null 2>&1; fi # stop blinking green white
		led_flash_pid=0
	fi
	...
	rm -f /tmp/zt_interface
	write_sc2_diag wifi "" "$sc2_last_error"
	sc2_last_error=""
